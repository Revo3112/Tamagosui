...
const ACCESSORY_HAT_IMAGE_URL: vector<u8> =
    b"https://copper-far-firefly-220.mypinata.cloud/ipfs/bafybeifj7rukic6j34zbdtqks2tuf27ufpecoyihpqj2jp2i5ehl43t3ry";

const EQUIPPED_GLASSES_KEY: vector<u8> = b"equipped_glasses";
const EQUIPPED_HAT_KEY: vector<u8> = b"equipped_hat";
const SLEEP_STARTED_AT_KEY: vector<u8> = b"sleep_started_at";

// === Accessory Types ===
const ACCESSORY_TYPE_GLASSES: vector<u8> = b"glasses";
const ACCESSORY_TYPE_HAT: vector<u8> = b"hat";
...
public entry fun mint_accessory(accessory_type: String, ctx: &mut TxContext) {
    ...
    let (name, image_url) = if (accessory_type_bytes == ACCESSORY_TYPE_GLASSES) {
        ...
        (string::utf8(b"Stylish Hat"), string::utf8(ACCESSORY_HAT_IMAGE_URL))
    } else {
        abort 0 // Invalid accessory type
    };
...
public entry fun equip_accessory(pet: &mut Pet, accessory: PetAccessory) {
    ...
    let key = if (accessory_type_bytes == ACCESSORY_TYPE_GLASSES) {
        ...
        string::utf8(EQUIPPED_HAT_KEY)
    } else {
        abort 0 // Invalid accessory type
    };

    // Check if this type of accessory is already equipped
    assert!(!dynamic_field::exists_<String>(&pet.id, copy key), E_ACCESSORY_TYPE_ALREADY_EQUIPPED);

    // Add accessory to pet
    dynamic_field::add(&mut pet.id, key, accessory);
    // Update image
    update_pet_image(pet);
    emit_action(pet, b"equipped_accessory");
}

public entry fun unequip_accessory(pet: &mut Pet, accessory_type: String, ctx: &mut TxContext) {
    assert!(!is_sleeping(pet), E_PET_IS_ASLEEP);
    ...
    emit_action(pet, b"unequipped_accessory");
}
...
public fun has_glasses_equipped(pet: &Pet): bool {
    let key = string::utf8(EQUIPPED_GLASSES_KEY);
    dynamic_field::exists_<String>(&pet.id, key)
}

public fun has_hat_equipped(pet: &Pet): bool {
    let key = string::utf8(EQUIPPED_HAT_KEY);
    dynamic_field::exists_<String>(&pet.id, key)
}

public fun get_equipped_accessories(pet: &Pet): (bool, bool) {
    (has_glasses_equipped(pet), has_hat_equipped(pet))
}

public fun get_equipped_glasses(pet: &Pet): &PetAccessory {
    let key = string::utf8(EQUIPPED_GLASSES_KEY);
    dynamic_field::borrow<String, PetAccessory>(&pet.id, key)
}

public fun get_equipped_hat(pet: &Pet): &PetAccessory {
    let key = string::utf8(EQUIPPED_HAT_KEY);
    dynamic_field::borrow<String, PetAccessory>(&pet.id, key)
}
